...
{
    // pthread_t是POSIX线程库的线程句柄，用于声明线程ID
    pthread_t thread;
    
    // 创建训练参数结构体对象指针，并动态分配内存（使用calloc函数，并没有使用malloc函数，表示清空内存中之前的东西，初始化为0）
    ...
    // 属Linux系统下的POSIX线程库。在Linux中将其作为一个库来使用，因此需要加上 -lpthread（旧编译器）
    // （或-pthread）以显式链接该库（darknet的Makefile文件中有加入）。函数在执行错误时的错误信息将作为返回值返回，
    // 并不修改系统全局变量errno，当然也无法使用perror()打印错误信息。
    // 第一个参数为指向线程标识符的指针；第二个参数用来设置线程属性，此处为0，
    // 第三个参数是线程运行函数的起始地址，此处将thread绑定train_thread()函数，
    // 最后一个参数是运行函数的参数。
    // 若线程创建成功，则返回0。若线程创建失败，则返回出错编号，并且*thread中的内容是未定义的。
    // 更为详细的可以Google或者百度之（可参考：http://www.yolinux.com/TUTORIALS/LinuxTutorialPosixThreads.html）
    // train_thread()函数，在本文件中定义，只有一个输入参数：训练参数指针ptr，该函数会调用train_network()进行训练
